...
public class AutomationService {
    ...
    private static final Set<String> API_AUTH_TYPES =
            Set.of("NONE", "BASIC", "BEARER_TOKEN", "API_KEY");

    private static final Set<String> API_KEY_LOCATIONS =
            Set.of("HEADER", "QUERY");
    ...
    private void validateApiConfiguration(
        ...
        if (actionType == null || !isApiRequestAction(actionType)) {
            return;
        }
        ...
        final JsonNode root;
        try {
            root = OBJECT_MAPPER.readTree(apiConfig);
        } catch (Exception exception) {
            throw new AutomationValidationException(
                    "API configuration must be valid JSON"
            );
        }

        if (root == null || !root.isObject()) {
            throw new AutomationValidationException(
                    "API configuration must be a JSON object"
            );
        }
        ...
        String type = auth.path("type").asText("NONE").trim().toUpperCase();
        if (!API_AUTH_TYPES.contains(type)) {
            throw new AutomationValidationException(
        ...
        switch (type) {
            ...
            case "BASIC" -> {
                requireAuthText(auth, "username",
                        "BASIC authentication requires a username");
                requireSecretReference(auth, "passwordSecretRef",
                        "BASIC authentication requires passwordSecretRef using ${ENV_NAME}");
            }
            case "BEARER_TOKEN" ->
                    requireSecretReference(auth, "tokenSecretRef",
                            "BEARER_TOKEN authentication requires tokenSecretRef using ${ENV_NAME}");
            case "API_KEY" -> {
                requireAuthText(auth, "keyName",
                        "API_KEY authentication requires a key name");
                requireSecretReference(auth, "valueSecretRef",
                        "API_KEY authentication requires valueSecretRef using ${ENV_NAME}");

                String location = auth.path("location")
                        .asText("HEADER")
                        .trim()
                        .toUpperCase();
                if (!API_KEY_LOCATIONS.contains(location)) {
                    throw new AutomationValidationException(
                            "API_KEY location must be HEADER or QUERY"
                    );
                }
            }
    ...
    private void requireAuthText(
        ...
        if (auth.path(field).asText("").isBlank()) {
            throw new AutomationValidationException(message);
        }
    }
    ...
    private void requireSecretReference(
        ...
        String value = auth.path(field).asText("").trim();
        if (!SECRET_REFERENCE_PATTERN.matcher(value).matches()) {
            throw new AutomationValidationException(message);
        }
    }

    private boolean isApiRequestAction(
            com.testforge.testforge_backend.automation.model.AutomationActionType actionType
    ) {
        return switch (actionType) {
            case API_GET, API_POST, API_PUT, API_PATCH, API_DELETE -> true;
            default -> false;
        };
    }
